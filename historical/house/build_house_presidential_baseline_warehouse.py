...
def build_validation_report(
    ...
    # The source does not provide 2012 presidential results
    # recomputed on Florida's court-ordered remedial congressional
    # map first used in 2016. Preserve these rows as unavailable
    # rather than imputing values or using simultaneous 2016 results.
    expected_missing_mask = (
        baselines["forecast_cycle"].eq(2016)
        & baselines["state"].eq("FL")
        & ~baselines[
            "presidential_baseline_available"
        ]
    )

    unexpected_missing = baselines.loc[
        ~baselines[
            "presidential_baseline_available"
        ]
        & ~expected_missing_mask
    ...
    expected_florida_missing_count = int(
        expected_missing_mask.sum()
    )

    if expected_florida_missing_count != 27:
        failures.append(
            "Expected exactly 27 unavailable Florida baselines "
            "for the 2016 remedial map; found "
            f"{expected_florida_missing_count}."
        )

    if not unexpected_missing.empty:
        missing_by_cycle = (
            unexpected_missing.groupby(
                "forecast_cycle"
            )
            .size()
        )
        ...
        for cycle, missing_count in (
            missing_by_cycle.items()
        ):
            failures.append(
                f"{cycle}: {missing_count} unexpected required "
    ...
    report_lines = [
        ...
        (
            "The source contains no 2012 presidential estimates on "
            "Florida's court-ordered congressional map first used in "
            "2016. All 27 Florida rows remain explicitly unavailable "
            "and must be excluded from 2016 baseline scoring unless a "
            "separate authoritative reconstruction is added."
        ),
        "",
        "Validation status:",
    ]
