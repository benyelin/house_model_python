...
The builder combines validated historical feature warehouses for:
...
def add_feature_availability_flags(frame: pd.DataFrame) -> pd.DataFrame:
    out = frame.copy()
    ...
    dna_columns = [
        column for column in out.columns if column.startswith("dna_")
    ]
    elasticity_columns = [
        column for column in out.columns
        if column.startswith("elasticity_")
    ]

    out["district_dna_feature_available"] = (
        out[dna_columns].notna().any(axis=1)
        if dna_columns
        else False
    )

    out["district_elasticity_feature_available"] = (
        out[elasticity_columns].notna().any(axis=1)
        if elasticity_columns
        else False
    )

    out["candidate_registry_feature_available"] = (
        out.get("candidate_registry_available", False)
        .fillna(False)
        .astype(bool)
    )
...
def validate_final(frame: pd.DataFrame) -> None:
    ...
    if not frame["district_dna_feature_available"].all():
        missing = frame.loc[
            ~frame["district_dna_feature_available"],
            ["forecast_cycle", "race_id"],
        ].head(20)
        raise ValidationError(
            "District DNA coverage is not complete. "
            f"Examples: {missing.to_dict('records')}"
        )

    if not frame["district_elasticity_feature_available"].all():
        missing = frame.loc[
            ~frame["district_elasticity_feature_available"],
            ["forecast_cycle", "race_id"],
        ].head(20)
        raise ValidationError(
            "District elasticity coverage is not complete. "
            f"Examples: {missing.to_dict('records')}"
        )
...
def order_columns(frame: pd.DataFrame) -> pd.DataFrame:
    leading = [
        ...
        "district_dna_feature_available",
        "district_elasticity_feature_available",
...
def print_summary(frame: pd.DataFrame, output_path: Path) -> None:
    ...
    log(
        "District DNA coverage: "
        f"{int(frame['district_dna_feature_available'].sum()):,}"
        f" / {len(frame):,}"
    )
    log(
        "District elasticity coverage: "
        f"{int(frame['district_elasticity_feature_available'].sum()):,}"
        f" / {len(frame):,}"
    )
    log(
...
def build(output_path: Path) -> pd.DataFrame:
    ...
    log("Loading historical District DNA warehouse...")
    dna = load_district_dna()
    canonical = validated_left_merge(
        canonical,
        dna,
        on=["forecast_cycle", "race_id"],
        label="District DNA merge",
    )
    log()

    log("Loading historical district elasticity warehouse...")
    elasticity = load_district_elasticity()
    canonical = validated_left_merge(
        canonical,
        elasticity,
        on=["forecast_cycle", "race_id"],
        label="District elasticity merge",
    )
    log()
