...
def main() -> None:
    ...
    master["incumbency_data_complete"] = (
        master["dem_candidate"].fillna("").eq("")
        | master["dem_fec_candidate_id"].notna()
    ) & (
        master["gop_candidate"].fillna("").eq("")
        | master["gop_fec_candidate_id"].notna()
    )
    ...
    validation_lines = [
        ...
        (
            "Open seats inferred: "
            f"{int(master['open_seat'].sum())}"
        ),
        "",
        "Important limitation:",
        (
            "Open-seat status is currently inferred from matched FEC "
            "incumbency codes. Races with unresolved candidate identities "
            "must be validated before this field is treated as final."
        ),
    ]
