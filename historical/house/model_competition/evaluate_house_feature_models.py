...
SAFE_STRUCTURAL_FEATURES = (
    # Do not include 2024 presidential results or DRA election
    # composites in a 2022 backtest. Those may contain information
    # unavailable before the election being forecast.
)
...
FEATURE_SPECIFICATIONS = (
    ...
    FeatureSpecification(
        model_id="H_safe_full_district_dna",
        description=(
            "Elasticity, behavior, 2020 Census demographics, region, "
            "district type, and reliability. No post-2022 political data."
        ),
        categorical_features=(
            *BASE_CATEGORICAL,
            "selected_regime_reliability",
...
def evaluate_intercept_only(
    modeling: pd.DataFrame,
    fold_assignments: np.ndarray,
    error_sd: float,
) -> tuple[dict[str, object], pd.DataFrame]:
    """
    Predict each held-out district's residual using only the mean
    residual among the training folds.

    This isolates the gain from correcting Layer 2's overall bias.
    Feature models must beat this control to demonstrate incremental
    district-level predictive value.
    """
    ...
    if np.isnan(predicted_residual).any():
        raise RuntimeError(
            "Missing intercept-only out-of-fold predictions."
        )
    ...
    result = {
        "model_id": "0_intercept_only",
        "model_description": (
            "Out-of-fold mean Layer 2 residual; no district features."
        ),
        "alpha": np.nan,
        "rows": len(modeling),
...
def main() -> None:
    ...
    for metric in [
        ...
        comparison[
            f"intercept_only_{metric}"
        ] = float(
            intercept_row[metric]
        )

        comparison[
            f"change_vs_intercept_only_{metric}"
        ] = (
            comparison[metric]
            - float(
                intercept_row[metric]
    ...
    if len(comparison) != (
        len(FEATURE_SPECIFICATIONS) + 1
    ):
        failures.append(
            "Intercept control or a feature specification is missing."
        )
    ...
    report_columns = [
        ...
        "change_vs_intercept_only_mean_absolute_error",
        "rmse",
        "change_vs_layer_2_rmse",
        "change_vs_intercept_only_rmse",
        "winner_accuracy",
        "change_vs_layer_2_winner_accuracy",
        "brier_score",
        "change_vs_layer_2_brier_score",
        "change_vs_intercept_only_brier_score",
        "log_loss",
        "change_vs_layer_2_log_loss",
        "change_vs_intercept_only_log_loss",
        "predicted_win_count_error",
        "expected_win_count_error",
    ...
    report_lines = [
        ...
        "",
        "Intercept-only residual benchmark:",
        (
            "Mean out-of-fold residual correction: "
            f"{float(intercept_row['residual_prediction_mean']):+.6f}"
        ),
        (
            "MAE: "
            f"{float(intercept_row['mean_absolute_error']):.6f}"
        ),
        (
            "RMSE: "
            f"{float(intercept_row['rmse']):.6f}"
        ),
        (
            "Winner accuracy: "
            f"{float(intercept_row['winner_accuracy']):.6f}"
        ),
        (
            "Brier: "
            f"{float(intercept_row['brier_score']):.6f}"
        ),
        (
            "Log loss: "
            f"{float(intercept_row['log_loss']):.6f}"
        ),
        "",
        "Selected feature models and control:",
        comparison[
            report_columns
        ].to_string(
            index=False,
        ...
        (
            "MAE change versus intercept-only: "
            f"{float(winner['change_vs_intercept_only_mean_absolute_error']):+.6f}"
        ),
        (
        ...
        (
            "RMSE change versus intercept-only: "
            f"{float(winner['change_vs_intercept_only_rmse']):+.6f}"
        ),
        (
        ...
        (
            "Brier change versus intercept-only: "
            f"{float(winner['change_vs_intercept_only_brier_score']):+.6f}"
        ),
        (
        ...
        (
            "Log-loss change versus intercept-only: "
            f"{float(winner['change_vs_intercept_only_log_loss']):+.6f}"
        ),
        "",
        "Leakage and benchmark controls:",
        (
            "Post-2022 presidential results and DRA election composites "
            "are excluded. Every feature model is compared with both the "
            "original Layer 2 forecast and an out-of-fold intercept-only "
            "residual correction."
        ),
        "",
        ...
        (
            ...
            "held-out-election-cycle test. Features that beat the "
            "intercept-only control must still pass pseudo-out-of-sample "
            "testing across earlier election cycles."
        ),
        "",
        "Validation status:",
    ]
