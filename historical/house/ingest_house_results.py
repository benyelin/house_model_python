...
def build_cycle(cycle: int, raw_path: Path) -> tuple[pd.DataFrame, str]:
    ...
    cycle_rows = raw.loc[
        ...
        # The forecasting target is the 435 voting House seats.
        # Exclude nonvoting delegate jurisdictions such as Washington, D.C.
        & ~raw["state_po"].astype(str).str.upper().isin(
            {"DC", "AS", "GU", "MP", "PR", "VI"}
        )
    ].copy()
    ...
    # Some uncontested historical races use -1 as a sentinel for an
    # unreported vote total. Negative values are not literal vote counts.
    cycle_rows["candidatevotes"] = (
        cycle_rows["candidatevotes"].clip(lower=0).astype(int)
    )
    cycle_rows["totalvotes"] = (
        cycle_rows["totalvotes"].clip(lower=0).astype(int)
    )
    ...
    for race_id, group in cycle_rows.groupby("race_id", sort=True):
        ...
        if total_votes <= 0:
            if structure["general_election_party_structure"] == "D_unopposed":
                actual_dem_margin = 100.0
                dem_vote_share = 100.0
                gop_vote_share = 0.0
            elif structure["general_election_party_structure"] == "R_unopposed":
                actual_dem_margin = -100.0
                dem_vote_share = 0.0
                gop_vote_share = 100.0
            else:
