...
def main() -> None:
    ...
    # The 2022 backtest input may already contain WAR-derived fields
    # from an earlier production-pipeline run. Remove them before merging
    # the leakage-safe historical WAR audit so the sensitivity test uses
    # exactly one authoritative version of each WAR field.
    stale_war_columns = [
        "district_id_war",
        "forecast_cycle",
        "dem_war_matched",
        ...
        "dem_candidate_war",
        "gop_candidate_war",
        "dem_observation_reliability_multiplier",
        "gop_observation_reliability_multiplier",
        ...
        "house_candidate_war_cap",
        "house_candidate_war_incumbent_discount",
        "house_candidate_war_one_sided_multiplier",
        "house_candidate_war_observation_prior_strength",
        ...
        "target_include_in_major_party_margin_scoring",
    ]

    races = races.drop(
        columns=[
            column
            for column in stale_war_columns
