...
def construct_retained_elasticity(
    ...
    """
    Retain a configurable share of predicted variation around 1.0.

    retention_level = 0.0:
        every district receives elasticity 1.0

    retention_level = 1.0:
        full characteristics prediction is retained
    """
    ...
    if (
        not np.isfinite(scored_mean_before_normalization)
    ...
    # Re-center within the scored sample so this test isolates
    # heterogeneous responsiveness rather than changing the overall
    # national-environment coefficient.
    normalized = (
        retained
        / scored_mean_before_normalization
    )

    return normalized, scored_mean_before_normalization


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Test characteristics-based House district elasticity "
            "against the established 2022 Layer 2 benchmark."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Comma-separated shares of characteristics-based "
            "elasticity variation to retain."
        ),
    )
    ...
    if not args.input_path.exists():
        raise FileNotFoundError(
            f"Missing characteristics Layer 3 input: {args.input_path}"
        )

    if not args.environment_path.exists():
        raise FileNotFoundError(
            f"Missing national environment: {args.environment_path}"
        )
    ...
    for retention in args.retention_levels:
        ...
        layer_3_summary = score_layer(
            scored_races=scored,
            model_name=(
                "layer_3_characteristics_elasticity_"
        ...
        row = {
            "retention_level": retention,
            "shrinkage_toward_one": 1.0 - retention,
        ...
        detail = pd.DataFrame(
            {
                ...
                "characteristics_elasticity_original": (
                    work["characteristics_elasticity"]
                ),
    ...
    sensitivity = sensitivity.sort_values(
        ["combined_rank", "retention_level"]
    ).reset_index(drop=True)

    control = sensitivity.loc[
        np.isclose(
            sensitivity["retention_level"],
            0.0,
        )
    ]
    ...
    if control.empty:
        failures.append(
            "Retention level 0.0 is required as the Layer 2 control."
        )
        ...
        control_detail = district_detail.loc[
            np.isclose(
                district_detail["retention_level"],
                0.0,
            )
        ]
        ...
        if control_margin_difference > 1e-9:
            failures.append(
                "Zero-retention control is not identical to Layer 2."
            )

    best = sensitivity.iloc[0]

    report_lines = [
        "2022 House Characteristics Elasticity Sensitivity",
        "=" * 48,
        "",
        ...
        "Control validation:",
        (
            "Maximum absolute margin difference at zero retention: "
        ...
        f"Retention level: {best['retention_level']:.2f}",
        (
    ...
    sensitivity_path = (
        args.output_dir
        / "house_2022_characteristics_elasticity_sensitivity.csv"
    )

    district_detail_path = (
        args.output_dir
        / "house_2022_characteristics_elasticity_sensitivity_district_detail.csv"
    )

    validation_path = (
        args.output_dir
        / "house_2022_characteristics_elasticity_sensitivity_validation.txt"
    )
    ...
    print(report)
    print()
    print("Characteristics elasticity sensitivity")
    print("--------------------------------------")
