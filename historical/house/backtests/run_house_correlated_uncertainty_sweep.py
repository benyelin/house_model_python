#!/usr/bin/env python3
"""
Calibrate the House correlated Election Day uncertainty allocation.

This Version 2 script holds the marginal race-level Election Day
uncertainty constant while sweeping the allocation between:

    - a single chamber-wide correlated error
    - residual district-specific independent error

The chamber-wide correlated component is an aggregate research proxy
for the production model's national, state, region, district-type, and
education/race group effects. This stage estimates the total amount of
correlated uncertainty before attempting to partition it among those
sources.
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "backtests"
    / "outputs"
    / "correlated_uncertainty_calibration"
)

DEFAULT_POSTERIOR_TOTAL_SD = math.sqrt(
    5.0625 ** 2
    + 6.1875 ** 2
)
...
CURRENT_GROUPED_CORRELATED_SD = math.sqrt(23.203125)
CURRENT_GROUPED_RESIDUAL_SD = math.sqrt(
    6.75 ** 2
    - CURRENT_GROUPED_CORRELATED_SD ** 2
)
...
def simulate_chamber_distribution(
    ...
    correlated_sd: float,
    residual_district_sd: float,
    n_sims: int,
    seed: int,
) -> np.ndarray:
    """
    Run the same correlated-plus-residual Election Day simulation used
    ...
    residual_residual_district_sds = (
        float(residual_district_sd)
        * district_multipliers
    )

    rng = np.random.default_rng(seed)

    correlated_errors = rng.normal(
        0.0,
        float(correlated_sd),
        size=n_sims,
    )

    residual_district_errors = rng.normal(
        0.0,
        residual_residual_district_sds.reshape(
            1,
            len(replay_df),
        ),
        size=(
    ...
    simulated_margins = (
        ...
        + correlated_errors.reshape(
            n_sims,
            1,
        )
        + residual_district_errors
    )
...
def summarize_allocations(
    ...
    group_columns = [
        ...
        "is_current_grouped_correlation",
        ...
        rows.append(
            {
                ...
                "is_current_grouped_correlation": bool(
                    is_current
                ),
...
def validate_outputs(
    ...
    current_rows = summary.loc[
        summary["is_current_grouped_correlation"]
    ]

    if len(current_rows) != 1:
        raise AllocationSweepError(
            "Current grouped-correlation benchmark is missing or duplicated."
        )

    checks.append(
        "PASS: current grouped-correlation benchmark is represented"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep House correlated-versus-residual "
    ...
    parser.add_argument(
        "--posterior-total-sd",
        "--total-sd",
        dest="posterior_total_sd",
        type=float,
        default=DEFAULT_POSTERIOR_TOTAL_SD,
        help=(
            "Fixed marginal race-level posterior SD. "
            "--total-sd is retained as a compatibility alias."
        ),
    )
    ...
    if args.posterior_total_sd <= 0.0:
    ...
    for correlated_sd in DEFAULT_CORRELATED_GRID:
        ...
        if correlated_sd in seen:
            continue

        seen.add(correlated_sd)

        if correlated_sd > args.posterior_total_sd:
            raise AllocationSweepError(
                "Correlated SD cannot exceed total SD: "
                f"{correlated_sd:.6f} > {args.posterior_total_sd:.6f}"
            )

        residual_district_sd = math.sqrt(
            max(
                0.0,
                args.posterior_total_sd ** 2
                - correlated_sd ** 2,
            )
        )
        ...
        allocation_rows.append(
            {
                ...
                "residual_district_error_sd": residual_district_sd,
            }
        )
    ...
    for allocation_number, allocation in enumerate(
        ...
        allocation_id = (
            f"national_{correlated_sd:.4f}"
            f"_district_{residual_district_sd:.4f}"
        )
        ...
        print(
            f"Residual district SD: {residual_district_sd:.4f}"
        )
        print(
            f"Total SD: {args.posterior_total_sd:.4f}"
        )
        print("=" * 72)
        ...
        for cycle_index, cycle in enumerate(
            ...
            simulated_seats = (
                simulate_chamber_distribution(
                    ...
                    residual_district_sd=residual_district_sd,
                    n_sims=args.sims,
            ...
            row = {
                ...
                "marginal_total_error_sd": float(
                    math.sqrt(
                        correlated_sd ** 2
                        + residual_district_sd ** 2
                    )
                ),
                "is_current_grouped_correlation": bool(
                    math.isclose(
                        correlated_sd,
                        CURRENT_GROUPED_CORRELATED_SD,
                        abs_tol=1e-9,
                    )
                ),
                "is_empirical_residual_allocation": bool(
    ...
    cycle_path = (
        args.output_dir
        / "house_correlated_uncertainty_by_cycle.csv"
    )

    summary_path = (
        args.output_dir
        / "house_correlated_uncertainty_summary.csv"
    )

    ranking_path = (
        args.output_dir
        / "house_correlated_uncertainty_ranking.csv"
    )

    config_path = (
        args.output_dir
        / "house_correlated_uncertainty_config.json"
    )

    validation_path = (
        args.output_dir
        / "house_correlated_uncertainty_validation.txt"
    )
    ...
    config = {
        ...
        "posterior_total_error_sd": float(args.posterior_total_sd),
        "correlated_sd_grid": [
            float(row["correlated_error_sd"])
            for row in allocation_rows
        ],
        "residual_residual_district_sd_grid": [
            float(row["residual_district_error_sd"])
            for row in allocation_rows
        ],
        "n_sims": int(args.sims),
        "seed": int(args.seed),
        ...
        "current_production_residual_district_sd": (
            CURRENT_GROUPED_RESIDUAL_SD
        ),
        "empirical_residual_correlated_sd": 1.9,
    }
