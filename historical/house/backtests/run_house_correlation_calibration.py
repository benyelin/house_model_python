...
class CorrelationCalibrationError(RuntimeError):
...
def resolve_fixed_control(df: pd.DataFrame) -> np.ndarray:
    ...
    if invalid:
        raise CorrelationCalibrationError(
...
def main() -> None:
    ...
    cycle_path = (
        args.output_dir
        / "house_correlation_calibration_by_cycle.csv"
    )
    summary_path = (
        args.output_dir
        / "house_correlation_calibration_summary.csv"
    )
    loo_path = (
        args.output_dir
        / "house_correlation_calibration_loo.csv"
    )
    config_path = (
        args.output_dir
        / "house_correlation_calibration_config.json"
    )
    validation_path = (
        args.output_dir
        / "house_correlation_calibration_validation.txt"
    )
    ...
    config = {
        "calibration_version": (
            "house_correlation_calibration_v1"
        ),
    ...
    validation_text = "\n".join(
        [
            "House Correlation Calibration Validation",
            "=" * 40,
            *validation_checks,
            "",
            "VALIDATION PASSED",
