...
def choose_match(
    ...
    same_race_candidates = fec.loc[
        fec["race_id"].eq(mit_row["race_id"])
        & fec["party_code"].eq(mit_row["party_code"])
    ].copy()

    def score_candidates(candidate_pool: pd.DataFrame) -> list[dict[str, object]]:
        scored_rows = []
        ...
        for index, row in candidate_pool.iterrows():
            ...
            # Prefer active/statutory candidates only as a small tie-breaker.
            status_bonus = 0.0
    ...
    candidates = same_race_candidates
    match_scope = "same_race"

    scored = score_candidates(candidates) if not candidates.empty else []

    best_same_race_raw_score = (
        float(scored[0]["raw_name_score"])
        if scored
        else 0.0
    )

    # FEC district codes can be stale or reflect a pre-redistricting seat.
    # When the race-level search fails, search the same state and party.
    if best_same_race_raw_score < 45.0:
        state_candidates = fec.loc[
            fec["state"].eq(mit_row["state"])
            & fec["party_code"].eq(mit_row["party_code"])
        ].copy()

        state_scored = score_candidates(state_candidates)

        if state_scored:
            state_best_raw_score = float(
                state_scored[0]["raw_name_score"]
            )

            if state_best_raw_score > best_same_race_raw_score:
                candidates = state_candidates
                scored = state_scored
                match_scope = "same_state_party"
    ...
    if not scored:
        return {
            ...
            "match_reason": "no_candidate_pool",
    ...
    if match_scope == "same_race":
        ...
        # Cross-district state fallback requires stronger name evidence.
        if raw_name_score >= 90 and score_gap >= 10:
            method = "high_confidence_state_fallback"
            retained_row = best_row
        elif raw_name_score >= 80 and score_gap >= 15:
            method = "probable_state_fallback"
            retained_row = best_row
        elif raw_name_score >= 65:
            method = "review_required_state_fallback"
            retained_row = best_row
        else:
            method = "unmatched"
            retained_row = None
    ...
    reason_with_scope = (
        best["reason"] + f"+scope_{match_scope}"
    )
    ...
    return {
        ...
        "match_method": method,
        "match_reason": reason_with_scope,
...
def main() -> None:
    ...
    high_confidence = registry["match_method"].isin(
        [
            ...
            "high_confidence_state_fallback",
        ]
    )
